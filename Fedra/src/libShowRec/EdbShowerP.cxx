...
void EdbShowerP::BuildShowerAxis()
{
    ...
    if (std::isnan(eShowerAxisAngle) || isinf(eShowerAxisAngle) )  {
        if (gEDBDEBUGLEVEL>3) {
            if (std::isnan(eShowerAxisAngle)) cout << "eShowerAxisAngle==isnan() " << endl;
...
void EdbShowerP::BuildParametrisation_YC()
{
    ...
    if (sizeb>0) {
        ...
        // (FWM, 10082009)
        if ( std::isnan(alphacalc[nbshower]) ) alphacalc[nbshower]=0.0;
